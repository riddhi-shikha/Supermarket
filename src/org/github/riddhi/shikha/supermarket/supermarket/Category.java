...
public class Category extends javax.swing.JFrame {
    ...
    public Category(int u_id, String name) {
        initComponents();
    ...
    private void initComponents() {
        ...
        CatId.setFont(new java.awt.Font("Cambria", 1, 14)); // NOI18N
        ...
        CatName.setFont(new java.awt.Font("Cambria", 1, 14)); // NOI18N
        ...
        CatDesc.setFont(new java.awt.Font("Cambria", 1, 14)); // NOI18N
        ...
        try {
            ...
            if(!search.getText().equals("Search by ID or Name")){
                if(StringUtils.isNumeric(search.getText())){
                    query = "SELECT c.cat_id AS ID, c.cat_name AS NAME, c.description AS DESCRIPTION, u.name AS CREATED_BY, c.created_on AS CREATED_ON," +
                        "(SELECT NAME FROM user WHERE u_id = (SELECT u_id FROM user WHERE u_id = c.modified_by)) AS MODIFIED_BY," +
                        "c.modified_on AS MODIFIED_ON " +
                        "FROM category c, user u WHERE c.created_by = u.u_id AND c.cat_id = "+Integer.parseInt(search.getText())+"";
                }else{
                    query = "SELECT c.cat_id AS ID, c.cat_name AS NAME, c.description AS DESCRIPTION, u.name AS CREATED_BY, c.created_on AS CREATED_ON," +
                        "(SELECT NAME FROM user WHERE u_id = (SELECT u_id FROM user WHERE u_id = c.modified_by)) AS MODIFIED_BY," +
                        "c.modified_on AS MODIFIED_ON " +
                        "FROM category c, user u WHERE c.created_by = u.u_id AND c.cat_name LIKE '"+search.getText()+"%'";
                }
            }else{
                query = "SELECT c.cat_id AS ID, c.cat_name AS NAME, c.description AS DESCRIPTION, u.name AS CREATED_BY, c.created_on AS CREATED_ON," +
                        "(SELECT NAME FROM user WHERE u_id = (SELECT u_id FROM user WHERE u_id = c.modified_by)) AS MODIFIED_BY," +
                        "c.modified_on AS MODIFIED_ON " +
                        "FROM category c, user u WHERE c.created_by = u.u_id";
            }
    ...
    private void AddBtnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_AddBtnMouseClicked
        if (CatName.getText().isEmpty() || CatDesc.getText().isEmpty()) {
            JOptionPane.showMessageDialog(this, "Missing Information");
        } else {
            try {
                
                con = DBConfig.getDBConnection();
                String query = "INSERT INTO category (cat_name, description, created_on, created_by)"
                                    + " values ('"+CatName.getText()+"', '"+CatDesc.getText()+"', '"+Utilities.getSQLDateWithTime()+"', "+uId+")";
                ...
                if(row == 1){
                    ...
                    JOptionPane.showMessageDialog(this, "Category Additaion Failed");
                }
        ...
        if (CatId.getText().isEmpty()) {
            ...
            if(res == JOptionPane.YES_OPTION){
                try {
                    ...
                    String Query = "DELETE FROM category WHERE cat_id = " + CId;
    ...
    private void UpdateBtnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_UpdateBtnMouseClicked
        if (CatId.getText().isEmpty() || CatName.getText().isEmpty() || CatDesc.getText().isEmpty()) {
            ...
            try {
                ...
                con = DBConfig.getDBConnection();
                String Query = "UPDATE category set cat_name = '" + CatName.getText() + "'"
                        + ", description = '" + CatDesc.getText() + "', modified_on = '"+Utilities.getSQLDateWithTime()+"',"
                        + " modified_by = "+uId+" WHERE cat_id = " + CatId.getText();
                Statement st = con.createStatement();
